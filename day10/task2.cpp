...
int calc(VVC input) {
  ...
  int biggest = 0;
  char Spipe = 'S';
  VVC visited_mask(input.size(), VC(input[0].size()));
  for (char pipe : pipes) {
    REP(i,0,input.size()) {
    ...
    dfs(S, S, g, visited);
    if ((visited[S.first][S.second] + 1) / 2 > biggest) {
      biggest = (visited[S.first][S.second] + 1) / 2;
      Spipe = pipe;
      REP(i,0,visited.size()) {
        REP(j,0,visited[0].size()) {
          visited_mask[i][j] = visited[i][j] != 0 ? input[i][j] : '.';
        }
      }
    }
  }
  ...
  // prev F -> rim
  int count = 0;
  REP(i,0,visited_mask.size()) {
    char prev = '.';
    bool inside = false;
    REP(j,0,visited_mask[0].size()) {
      char c = visited_mask[i][j];
      if (c == '|') {
        inside = !inside;
      }
      else if (c == 'L' || c == 'F') {
